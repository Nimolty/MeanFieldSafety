...
from Algorithms.SDE import MiniUpdate, marginal_prob_std
...
def snapshot(env, file_name, camera_hight=1.0):
    img = env.render(256, camera_hight)
...
def comp_sup_vel(state, t, score, edge_func):
    state_inp = torch.from_numpy(state).to(device)
    with torch.no_grad():
        edge = edge_func(state_inp).to(device)
        state_ts = Data(x=state_inp, edge_index=edge)
        inp_batch = Batch.from_data_list([state_ts])
        vels = score(inp_batch, t) # 我们normalize了sup vel就发挥不出他的威力了！
    return vels.cpu().numpy()
...
def plan():
    ...
    edge_func = partial(radius_graph, r=R)
    t = torch.tensor([t0]).unsqueeze(0).to(device)
    get_sup_vel = partial(comp_sup_vel, score=score_support.to(device), edge_func=edge_func)
    sup_vel = get_sup_vel(init_state, t)
