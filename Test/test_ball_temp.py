...
from Algorithms.SDE import ScoreModelGNN, ScoreModelGNNMini, MiniUpdate, MiniDouble, marginal_prob_std, diffusion_coeff
...
def plan():
    ...
    for step in tqdm(range(total_steps)):
        try:
            ...
            if IS_GOAL:
                if IS_SUPPORT:
                    # print(np.mean(np.abs(sup_vels * SUP_RATE)))
                    # print(np.mean(np.abs(normalise_vels(tar_vels))))
                    # set_trace()
